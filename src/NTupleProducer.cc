...
NTupleProducer::NTupleProducer(const edm::ParameterSet& iConfig){
  ...
  for (size_t i=0; i<jConfigs.size(); ++i)
    if ( jConfigs[i].getUntrackedParameter<bool>("isPat") ) 
      jetFillers.push_back( new JetFillerPat(jConfigs[i], fEventTree, fIsPat, fIsRealData) );
    else
...
void NTupleProducer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup){
  ...
  for (vector<OrderPair>::const_iterator it = pfmuOrdered.begin();
    ...
    // MC Matching
    if(!fIsRealData){
      vector<const GenParticle*> PfMuMatch = matchRecoCand(&muon, iEvent);
      if(PfMuMatch[0] != NULL){
        fTGenPfMuId[pfmqi]       = PfMuMatch[0]->pdgId();
        fTGenPfMuStatus[pfmqi]   = PfMuMatch[0]->status();
        fTGenPfMuCharge[pfmqi]   = PfMuMatch[0]->charge();
        fTGenPfMuPt[pfmqi]       = PfMuMatch[0]->pt();
        fTGenPfMuEta[pfmqi]      = PfMuMatch[0]->eta();
        fTGenPfMuPhi[pfmqi]      = PfMuMatch[0]->phi();
        fTGenPfMuE[pfmqi]        = PfMuMatch[0]->energy();

        fTGenPfMuMId[pfmqi]      = PfMuMatch[1]->pdgId();
        fTGenPfMuMStatus[pfmqi]  = PfMuMatch[1]->status();
        fTGenPfMuMCharge[pfmqi]  = PfMuMatch[1]->charge();
        fTGenPfMuMPt[pfmqi]      = PfMuMatch[1]->pt();
        fTGenPfMuMEta[pfmqi]     = PfMuMatch[1]->eta();
        fTGenPfMuMPhi[pfmqi]     = PfMuMatch[1]->phi();
        fTGenPfMuME[pfmqi]       = PfMuMatch[1]->energy();

        fTGenPfMuGMId[pfmqi]     = PfMuMatch[2]->pdgId();
        fTGenPfMuGMStatus[pfmqi] = PfMuMatch[2]->status();
        fTGenPfMuGMCharge[pfmqi] = PfMuMatch[2]->charge();
        fTGenPfMuGMPt[pfmqi]     = PfMuMatch[2]->pt();
        fTGenPfMuGMEta[pfmqi]    = PfMuMatch[2]->eta();
        fTGenPfMuGMPhi[pfmqi]    = PfMuMatch[2]->phi();
        fTGenPfMuGME[pfmqi]      = PfMuMatch[2]->energy();
      }
      PfMuMatch.clear();
    }
  }
  ...
  for ( View<pat::Electron>::const_iterator Eit = pfelectrons->begin(); Eit != pfelectrons->end(); 
    ...
    if(pfeqi >= gMaxneles){
      edm::LogWarning("NTP") << "@SUB=analyze()"
                             << "Maxielm number of electrons exceeded";
      fTflagmaxelexc = 1;
      fTgoodevent = 1;
      break;
    }
  ...
  for (vector<OrderPair>::const_iterator it = pfelOrdered.begin();
    ...
    // Combined methods for Global and Tracker electrons:
    ...
    // MC Matching
    if(!fIsRealData){
      vector<const GenParticle*> PfelMatch = matchRecoCand(&electron, iEvent);
      if(PfelMatch[0] != NULL){
        fTGenPfElId[pfeqi]       = PfelMatch[0]->pdgId();
        fTGenPfElStatus[pfeqi]   = PfelMatch[0]->status();
        fTGenPfElCharge[pfeqi]   = PfelMatch[0]->charge();
        fTGenPfElPt[pfeqi]       = PfelMatch[0]->pt();
        fTGenPfElEta[pfeqi]      = PfelMatch[0]->eta();
        fTGenPfElPhi[pfeqi]      = PfelMatch[0]->phi();
        fTGenPfElE[pfeqi]        = PfelMatch[0]->energy();

        fTGenPfElMId[pfeqi]      = PfelMatch[1]->pdgId();
        fTGenPfElMStatus[pfeqi]  = PfelMatch[1]->status();
        fTGenPfElMCharge[pfeqi]  = PfelMatch[1]->charge();
        fTGenPfElMPt[pfeqi]      = PfelMatch[1]->pt();
        fTGenPfElMEta[pfeqi]     = PfelMatch[1]->eta();
        fTGenPfElMPhi[pfeqi]     = PfelMatch[1]->phi();
        fTGenPfElME[pfeqi]       = PfelMatch[1]->energy();

        fTGenPfElGMId[pfeqi]     = PfelMatch[2]->pdgId();
        fTGenPfElGMStatus[pfeqi] = PfelMatch[2]->status();
        fTGenPfElGMCharge[pfeqi] = PfelMatch[2]->charge();
        fTGenPfElGMPt[pfeqi]     = PfelMatch[2]->pt();
        fTGenPfElGMEta[pfeqi]    = PfelMatch[2]->eta();
        fTGenPfElGMPhi[pfeqi]    = PfelMatch[2]->phi();
        fTGenPfElGME[pfeqi]      = PfelMatch[2]->energy();
      }
      PfelMatch.clear();
    }
  }
  ...
  for ( View<pat::Tau>::const_iterator Tit = pftaus->begin(); Tit != pftaus->end(); 
    ...
    if(pftqi >= gMaxneles){
      edm::LogWarning("NTP") << "@SUB=analyze()"
                             << "Maxielm number of taus exceeded";
      fTflagmaxelexc = 1;
      fTgoodevent = 1;
      break;
    }
  ...
  for (vector<OrderPair>::const_iterator it = pftauOrdered.begin();
    ...
    if(!fIsRealData){
      ...
      if(PftauMatch[0] != NULL){
        ...
        fTGenPfTauCharge[pftqi]   = PftauMatch[0]->charge();
        fTGenPfTauPt[pftqi]       = PftauMatch[0]->pt();
        fTGenPfTauEta[pftqi]      = PftauMatch[0]->eta();
        fTGenPfTauPhi[pftqi]      = PftauMatch[0]->phi();
        fTGenPfTauE[pftqi]        = PftauMatch[0]->energy();

        fTGenPfTauMId[pftqi]      = PftauMatch[1]->pdgId();
        fTGenPfTauMStatus[pftqi]  = PftauMatch[1]->status();
        fTGenPfTauMCharge[pftqi]  = PftauMatch[1]->charge();
        fTGenPfTauMPt[pftqi]      = PftauMatch[1]->pt();
        fTGenPfTauMEta[pftqi]     = PftauMatch[1]->eta();
        fTGenPfTauMPhi[pftqi]     = PftauMatch[1]->phi();
        fTGenPfTauME[pftqi]       = PftauMatch[1]->energy();

        fTGenPfTauGMId[pftqi]     = PftauMatch[2]->pdgId();
        fTGenPfTauGMStatus[pftqi] = PftauMatch[2]->status();
        fTGenPfTauGMCharge[pftqi] = PftauMatch[2]->charge();
        fTGenPfTauGMPt[pftqi]     = PftauMatch[2]->pt();
        fTGenPfTauGMEta[pftqi]    = PftauMatch[2]->eta();
        fTGenPfTauGMPhi[pftqi]    = PftauMatch[2]->phi();
        fTGenPfTauGME[pftqi]      = PftauMatch[2]->energy();
      }
      PftauMatch.clear();
    }
  }
